#!/usr/bin/env python3
"""One bounded llama.cpp shared-TP2 C2 128K run for authoritative WBS 3 v2.

Runs exactly one model/lane combination. It starts one TP2 llama-server with two
128K logical slots, materializes the frozen concurrency/v2 workload through the
live tokenizer, sends exactly one synchronized C2 batch, preserves runtime
overlap/queue evidence, then cleans up only the owned container.
"""
...
MODEL_WBS = {
    "qwen3.8-27b": "3.1.1",
    "ornith-1.5-9b": "3.1.2",
    "ornith-1.5-35b-a3b": "3.1.3",
    "gemma4-26b-a4b": "3.1.4",
}
...
def measure(raw):
    ...
    adapter = h.HTTPAdapter(config["endpoint"], "llama.cpp", timeout_s=3600)
...
def run_locked(args):
    ...
    plan = launcher.build_plan(
        ROOT, args.model, args.lane, 2, "tp2-shared", args.port
    )
    ...
    cmd[2:2] = [
        "--cidfile", str(cidfile),
        "--label", "experiment=" + args.experiment_id,
    ]
    ...
    config = future_config(
        ...
        | dict(
            ...
            notes=(
                f"WBS {wbs}; authoritative concurrency/v2; one synchronized "
                "C2 batch; shared TP2; 2048 output reserve, minimum 256 per request; "
                "semantic oracle is pre-registered."
            ),
        )
    )
    if args.retry_of:
    ...
    checkpoint(
        ...
        measurement_timeout_s=4200,
        startup_timeout_s=300,
        scope=f"WBS {wbs}: authoritative v2 shared-TP2 C2 128K",
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--lane", choices=["TARGET", "NGRAM", "MTP", "MTP_NGRAM"]
    )
