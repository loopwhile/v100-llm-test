#!/usr/bin/env python3
"""One bounded 1Cat-vLLM 128K recovery validation run for Qwen3.8-27B.

This script executes exactly one measured inference request on Qwen3.8-27B
using the realistic diversified 128K multi-file codebase context, combining
the proven 128K capacity enablers (--language-model-only, gpu_memory_utilization=0.92,
decode partition 256) with the historically clean Qwen 1Cat execution path:
fp8_e5m2 KV, GDN Triton prefill, VLLM_SM70_GDN_DECODE_FLASHQLA=0, FLASH_ATTN_V100,
thinking=false, temperature=0, top_p=1, seed=38.
"""
...
def run_locked(args):
    ...
    # Override for 128K recovery validation:
    # 1. KV cache: fp8_e5m2
    # 2. max_num_batched_tokens: 2048
    # 3. additional-config: {"gdn_prefill_backend":"triton"}
    # 4. environment: VLLM_SM70_GDN_DECODE_FLASHQLA=0, VLLM_FLASH_V100_DECODE_PARTITION_SIZE=256
    # 5. sampling: temperature=0, top_p=1, seed=38, presence_penalty=0.0
    # 6. thinking: false
    cmd = list(plan["commands"][0])

    # Replace kv-cache-dtype
    if "--kv-cache-dtype" in cmd:
        idx = cmd.index("--kv-cache-dtype")
        cmd[idx + 1] = "fp8_e5m2"
    else:
        cmd.extend(["--kv-cache-dtype", "fp8_e5m2"])

    # Replace max-num-batched-tokens with 2048
    ...
    # Add --additional-config '{"gdn_prefill_backend":"triton"}'
    if "--additional-config" in cmd:
        idx = cmd.index("--additional-config")
        cmd[idx + 1] = '{"gdn_prefill_backend":"triton"}'
    else:
        cmd.extend(["--additional-config", '{"gdn_prefill_backend":"triton"}'])
    ...
    sampling = {"temperature": 0.0, "top_p": 1.0, "seed": 38, "presence_penalty": 0.0}

    chat_template = (
        "HF tokenizer chat template; enable_thinking=false; temperature=0.0; seed=38"
    )
    ...
    config = future_config(
        ...
        | dict(
            ...
            notes=(
                "128K recovery validation run: Qwen3.8-27B QUASAR NVFP4 + 1Cat-vLLM 1.5.0 on 2x V100 16GB TP2 "
                "with fp8_e5m2 KV, GDN Triton prefill, flashqla disabled, decode partition 256, LM-only, "
                "util 0.92, temperature=0, seed=38, thinking=false."
            ),
        )
    )
    ...
    checkpoint(
        ...
        scope="128K recovery validation: Qwen3.8-27B 1Cat-vLLM fp8_e5m2 GDN-triton clean-path",
...
def main():
    parser = argparse.ArgumentParser(
        description="One measured 1Cat-vLLM 128K recovery validation run for Qwen3.8-27B"
    )
    parser.add_argument("--experiment-id", required=True)
    parser.add_argument("--model", default="qwen3.8-27b")
    parser.add_argument("--port", type=int, default=18080)
    parser.add_argument("--workload", default="workloads/diagnostic/v100_q38_recovery_e5m2_128k.json")
